...
files_and_titles=$(
for d in $(find src/main/sketches -type d -mindepth 1 -maxdepth 1)
do
    title=$(basename $d)
    file=$(find $d -name sketch.js | head -n 1 | sed 's_src/main/sketches_._')
    echo export { default as $title } from \'$file\' >> src/main/sketches/sketches.js
